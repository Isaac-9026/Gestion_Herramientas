...
function createCatalogModule(config) {
  const { endpoint, singularLabel, pluralLabel } = config;
  ...
  return {
    ...
    _openModal(mode, item = null) {
      ...
      document.getElementById("catalogoNombre").value = isEdit
        ? item.nombre
        : "";
    ...
    _filter() {
      const search =
        document.getElementById("searchCatalogo")?.value.toLowerCase() || "";
    ...
    _bindEvents() {
      document
        .getElementById("btnNuevoCatalogo")
        ?.addEventListener("click", () => this.openNew());

      document
        .getElementById("btnSaveCatalogo")
        ?.addEventListener("click", () => this._save());

      document
        .getElementById("btnCancelCatalogo")
        ?.addEventListener("click", () => closeOverlay("modalCatalogoOverlay"));

      document
        .getElementById("btnCloseModalCatalogo")
        ?.addEventListener("click", () => closeOverlay("modalCatalogoOverlay"));

      document
        .getElementById("btnRefreshCatalogo")
        ?.addEventListener("click", () => this.load());

      document
        .getElementById("searchCatalogo")
        ?.addEventListener("input", () => this._filter());

      document
        .getElementById("modalCatalogoOverlay")
